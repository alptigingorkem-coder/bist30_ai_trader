...
def train_catboost_ranker():
    ...
    with mlflow.start_run(run_name="CatBoost_Daily"):
        ...
        if hasattr(ranker.model, 'get_best_score'):
            best_scores = ranker.model.get_best_score()
            if 'validation' in best_scores:
                for metric, value in best_scores['validation'].items():
                    mlflow.log_metric(f"best_{metric}", value)
        
        # Log model artifact
        mlflow.catboost.log_model(ranker.model, "model")
